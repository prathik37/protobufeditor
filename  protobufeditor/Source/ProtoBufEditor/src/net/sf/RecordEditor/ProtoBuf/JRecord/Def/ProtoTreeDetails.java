...
extends TreeDetails<ProtoFieldDef, ProtoRecordDef, ProtoLayoutDef, ProtoChildDefinition, ProtoLine> {
...
//						layout.getRecord(childDef.recordIndex).getProtoDesc().toProto().toBuilder()
	 ...
	 */
	@Override
	public void removeChild(AbstractLine<ProtoLayoutDef> child) {
		if (super.line != child.getTreeDetails().getParentLine()) {
			...
			Common.logMsg("Could not remove child, it is not present: \n" + child.getFullLine()  + "\n", null);
			return;
		}
		...
		if (childD.isRepeated()) {
			...
			Common.logMsg("Could not remove child, it is Required: \n"+ child.getFullLine() + "\n", null);
		}
	}
